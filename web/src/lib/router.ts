...
export type StockTab = "price" | "chips" | "fundamentals";
export type Route = { page: "home" } | { page: "market" } | { page: "stock"; stockId: string; tab: StockTab };

export function parse(hash: string): Route {
  if (hash === "#/market") return { page: "market" };
  const stock = /^#\/stock\/([0-9A-Za-z]+)(?:\/(chips|fundamentals))?$/.exec(hash);
  return stock ? { page: "stock", stockId: stock[1], tab: (stock[2] as StockTab | undefined) ?? "price" } : { page: "home" };
}
