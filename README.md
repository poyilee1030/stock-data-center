...
歷史產業分類（Step 39，ADR-0030）：39-a（MERGED #72）把兩個交易所的產業類別調整公告存進 `industry_changes`；39-b（IN REVIEW #73）把依類股查的每日行情存進 `industry_observations`，作為已結束掛牌期間的錨點與上櫃對帳，兩者都已在 `stockdc_backfill` 以真實來源回補。分類期間與 API（39-c）尚未開始，所以產業歷史還不能經 API 查詢。
