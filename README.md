...
財務報表（iXBRL）同樣拆成三部分：23-a 是 parser 與文件契約（已合併，只有解析，
不寫入資料庫）、23-b 是官方與檔案庫 adapter 及匯入路徑（本 step）、23-c 是全量
backfill 與對帳。

23-b 存的是**與舊資料庫相同的範圍**：資產負債表、綜合損益表、現金流量表三張表，
由文件自己的錨點界定。權益變動表、附註、附表與敘述區塊計數但不存，要不要存留到
ROADMAP 最後再決定。此時版本只帶 `unknown` 證據，只有 System PIT 看得到；發布證據
與全量 backfill 是 23-c，所以資料庫裡還不是完整的財務報表歷史。
