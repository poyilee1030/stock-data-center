...
ROADMAP §20 is the authoritative ledger. Its current snapshot identifies Steps 1–18, 19-a through 19-e, 20-a through 20-d, 21-a, 21-b, 22-a through 22-c, 23-a through 24-b, 35-a, 35-b-1, 35-c-1 through 35-c-4, 35-d-1 through 35-d-3, 26-b through 26-f, 27-a through 27-c, 36, 37-a through 37-c, 38-a and 39-a through 39-c as MERGED, and 26-a, 29, 32 and 35-b-2 as SUPERSEDED; 28-a is IN REVIEW; no step is currently marked `THIS STEP` (a contextual marker, not an additional status value):
...
Step 39-c industry classification: periods + API         MERGED (#74, industry-classifications)
```
...
The web dashboard (Step 37, ADR-0029) adds, under `web/` only:

```text
React 19 + TypeScript, built by Vite into static files
Apache ECharts 6
Vitest (pure functions), Playwright (end-to-end against the real API)
```

It reads data through the public API alone, served same-origin by the `api` container.
...
`technical_indicators_pit:v1` (delivered by Step 35-c-4 as `technical_indicators:v1`) stays computed on demand under full PIT (§15) as the reference implementation. Adjusted prices (Step 36) keep PIT corporate-action visibility (§51.3): `adjusted_prices_pit:v1` is computed on demand and never stored.
