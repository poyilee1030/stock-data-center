...
`src/` 新增 673 行，在 CLAUDE.md §1 的 800 行內。
...
| 可見性查詢：原始值 | PASS | 規則時刻前 1 秒不可見、規則時刻可見，即使 `recorded_at` 晚得多 |
...
從 `stockdc_backfill`（只讀）把這三天的 v2 資料、`stocks` 與引用到的 fetch 複製到
暫存資料庫，再以 `--refetch` 對 17 個 job 真實抓取，結束後刪除暫存資料庫。73 秒，
退出碼 0。
...
| `index_prices` | 378 | 378 | 5 |
| 合計 | 31,148 | 31,148 | 5 |

每個 (表, 來源, 日期) 的既有列數都等於該次抓取的「不變」列數，所以既有的每一列都在
官方檔案裡原樣出現。

新增的 5 列全部歸因到同一件事：TAIEX 的請求單位是整個月，2026-09 的檔案包含
`stockdc_backfill` 最後日期（09-16）之後的 09-17、18、21、22、23，它們沒有既有列可比；
腳本把這類列單獨列在 `beyond_migrated_history`，不算差異。
...
job 各 1,627 個交易日，待抓 0；TAIEX 81 個月，待抓 1（2026-09，尚未結算）。
...
- TAIEX 收盤檢查只在 `MI_INDEX` 已存該日時生效；先抓 TAIEX 月檔、後抓 `MI_INDEX` 時，
  由既有的 `scripts/reconcile_market_indices.py` 對帳涵蓋。
