...
| 標準 | 結果 | 證據 |
| --- | --- | --- |
| 舊系統 `foreign_holding` 在兩個市場都已對帳 | PASS | **TWSE：**比較 1,634,573 列，**差異為零**；另有 1,859 列落在舊系統檔案對不上期間內任何一天的兩個日期（2022-06-28、2024-12-19，一致率 0%）。**TPEx：**MOPS 比較 1,288,755 列，差異為零；`insti/qfii` 比較 1,295,291 列，差異全部已分類（見下文）。 |
...
**TWSE — `legacy_file_matches_no_date_in_window`：2 個日期，1,859 列。** 2022-06-28
和 2024-12-19，舊系統的值與我們同一天的值一致率為 0%，而且期間內沒有任何一個儲存
的日期能重現舊系統的列。錯在舊系統那邊，而不是我們，證據有兩項：我們這兩天的 raw
artifact 回應的日期就是請求的日期（adapter 強制檢查），我們的列也和前後交易日連續
（發行股數相同的比例為 97.7% 和 93.7%）。舊系統這兩個檔案的最後一欄全是 `0`，
0050 的發行股數分別是 495,500,000 和 895,500,000，看起來是 2020 年之前的資料；
抽樣 2012–2019 的 TWSE 回應沒有找到確切的日期，所以報告不宣稱它是哪一天。

**MOPS — `legacy_only:mops_omits_security_no_longer_listed`：7,528 列。** 上一節的
五支證券。對帳的規則有兩個條件：MOPS 在整段期間都沒有這支證券的任何一列，**而且**
`insti/qfii` 在同一天有這一列；缺一個就是未解釋。另 1 列（5236 的 2021-07-28）是
`legacy_only:mops_omits_security_no_longer_listed_before_first_trade`：那天在它第一個
TPEx 交易日（2021-07-29，依 Step 17 的價格）之前，`insti/qfii` 從第一個交易日才開始
列出它。
...
對 #33 的 `/code-review medium` 在對帳腳本中找到兩個問題，adapter、importer、CLI 和
migration 沒有發現問題。兩個問題都在改動任何東西之前對照程式確認過。

| # | 發現 | 驗證方式 | 處置 |
| --- | --- | --- | --- |
| 1 | MOPS 的 legacy_only 列沒有檢查 `insti/qfii` 就被歸為已解釋；docstring 和報告說的條件，程式沒有做，只把 `in_insti_qfii` 寫進樣本 | 閱讀：分類只看「MOPS 整段期間都沒有這支證券」 | **已修正。** `insti/qfii` 同一天沒有這一列時是未解釋的 `legacy_only`。重跑後 7,528 列確認 `insti/qfii` 有；5236 的 1 列浮現為未解釋，以 Step 17 價格的首個交易日為證據另立類別。 |
| 2 | `legacy_captured_another_date` 是假設，不是證明：一致率低於 50% 就整天歸類，沒有證明舊系統的列等於另一天，也無法排除是我們存錯了日期 | 修正後重跑：期間內沒有任何儲存的日期能重現舊系統這兩天的列，腳本以 1 結束 | **已修正。** 只有在另一個儲存的日期完全重現舊系統的列時才用這個類別；找不到時，只有在我們的列和前後交易日連續時，才歸為 `legacy_file_matches_no_date_in_window`，否則是未解釋。報告不再宣稱舊系統存了哪一天的檔案。 |
