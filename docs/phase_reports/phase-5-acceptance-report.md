...
| --- | --- | --- |
| Actual EPS period basis | PASS | `EPSPeriodBasis` freezes `quarter`, `ytd`, and `annual`; `actual_eps` requires one explicitly. A full-year Q4 fact resolves as annual and a quarter lookup returns no value. Quarter and annual Q4 values coexist; a valid longer-than-quarter Q3 context is classified as YTD. |
...
| Source/context EPS classification | PASS | `SourceContextClassification` requires a source context reference, versioned classifier rule, exact dates, and an explicit source role. `classify_eps_period_basis` validates that role against the normalized fact/filing; `basic_eps` writer input cannot rely on duration alone. |
...
| Dataset-specific regression tests exist | PASS | Twenty-one focused Phase 5 tests cover value/nil contracts, seal visibility/immutability, Q4/two-clock and basis-specific EPS, source/context classification, source-fact lineage, full context identity, duplicates, repeated lineage, cross-source rejection, migration backfill, competing revisions, correction/retraction, scope, and dependency boundaries. |
...
source role is authoritative; duration is defensive       PASS
invalid/suspicious short context rejected                 PASS
...
pytest:                     96 passed
Phase 5 focused tests:      21 passed (13 integration, 8 unit)
