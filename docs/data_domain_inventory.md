...
| `dividend` | date/symbol/name, close before event, reference price, rights/dividend value, action type | Stable source event identity in `corporate_action_events` plus observed `corporate_action_versions`; explicit cash/earnings-stock/capital-surplus-stock/split/reverse-split/rights/capital-reduction/ex-event types and all dates, share quantities, official prices, and source terms are revision content. |
...
| `corporate_action` | observed | stable event (`security`, source, source event key) | source-faithful type, dates, dividend components, old/new shares, rights terms, official reference values, and source terms revision | materialized | adjusted prices/returns |
