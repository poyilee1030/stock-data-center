...
| `valuation_daily` | close, official TTM EPS, official PE, official PE percentile, official ROE | Source-published PE is observed. Data Center-calculated TTM EPS/PE percentile/ROE and valuations are canonical derived `valuation_metrics:v1`; close is referenced from PIT-safe daily prices, not duplicated. |
...
| `corporate_action` | observed | security, source, action type, ex-date | action terms revision | materialized | adjusted prices/returns |
