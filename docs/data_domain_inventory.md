...
| --- | --- | --- |
| `stock_info` | `symbol`, `name`, `market`, industry/category, listing and delisting dates | `stocks` (identity: name, industry) and `listings` (one span per market with `listed_on`, `delisted_on`), ADR-0028: every common stock on today's ISIN list and every one delisted since 2020-01-02 that an official source proves common. Category is a filter, not a column: every row is a common stock. No name history is kept; industry history is the exchanges' reclassification announcements in `industry_changes` (Step 39, ADR-0030), from which the category periods are derived. The datasets are still fetched only for stocks listed today, so the survivorship bias is disclosed until Step 38-b. |
...
| `industry-classifications` (API) | periods derived at query time, not stored | from `industry_changes`, `industry_observations`, `listings`, `stocks` (ADR-0030 §3) | a period's start (category in use), a change's notice release rule, the 2023-03-28 category notice | industry exclusion, peer ranking, exposure |
