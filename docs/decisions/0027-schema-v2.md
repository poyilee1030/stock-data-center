...
- **增量計算**：仿照舊系統，從上次之後有新輸入列的最早日期往後重算（新的交易日與較早日期的更正是
  同一種情況），往前讀 500 個日曆日的暖機緩衝，且至少涵蓋最長視窗的 240 列。
- **增量的精度**（owner 決定，2026-09-24，Step 26-b 實測後）：指數類指標（K、D、RSI、MACD）永遠
  記得起點，500 天緩衝的增量結果與整段重算不逐位相同（全市場實測最大差：MACD 為收盤價的 4.1e-6，
  RSI 為 6.5e-8）。owner 選擇保留舊系統的緩衝，驗收改為容差：MACD 在收盤價的 1e-5 以內，K、D、RSI
  在 1e-6 以內；視窗類指標與連續天數逐位相同。整段重算（`--full`）沒有這個殘差，輸入沒有更正時與
  `technical_indicators_pit:v1` 逐位相同。
