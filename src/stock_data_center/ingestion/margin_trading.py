"""Margin-trading hooks for the shared raw-first import lifecycle.

One table per market and trade date, like the per-security flows. Every row
either stores or fails its file. Quantities are lots converted to shares.
"""
...
class MarginTradingImporter(
    RawFirstImporter[MarginTradingRequest, ParsedMarginTrading]
):
    """Import one market's per-security margin trading for one trade date."""
    ...
    def _source_scope(
        ...
        adapter: RawFirstAdapter[
            MarginTradingRequest, ParsedMarginTrading
        ],
        request: MarginTradingRequest,
    ...
    def _source_semantics(self, adapter) -> Mapping[str, object]:
        if not isinstance(adapter, MarginTradingAdapter):
            raise TypeError(
                "MarginTradingImporter requires an MarginTradingAdapter"
            )
        return {
    ...
    def _dataset_description(self, adapter) -> str:
        return "per-security margin purchases and short sales"
    ...
    def _write_business(
        ...
        *,
        ...
        written = self._writer.append_margin_tradings(
            connection,
