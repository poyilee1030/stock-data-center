"""Securities-lending hooks for the shared raw-first import lifecycle.

One table per market and trade date, like margin trading. Every row either
stores or fails its file. Quantities are published in shares and stored as such.
"""
...
class SecuritiesLendingImporter(
    RawFirstImporter[SecuritiesLendingRequest, ParsedSecuritiesLending]
):
    """Import one market's per-security securities lending for one trade date."""
    ...
    def _source_scope(
        ...
        adapter: RawFirstAdapter[
            SecuritiesLendingRequest, ParsedSecuritiesLending
        ],
        request: SecuritiesLendingRequest,
    ...
    def _source_semantics(self, adapter) -> Mapping[str, object]:
        if not isinstance(adapter, SecuritiesLendingAdapter):
            raise TypeError(
                "SecuritiesLendingImporter requires a SecuritiesLendingAdapter"
            )
        return {
            "market": adapter.market,
            "quantity_unit": "shares, as published",
            "columns": dict(adapter.columns),
            "next_limit": "融券 次一營業日限額 / 限額, exact shares",
            "adjustment": "signed, as published",
            "note": "備註 without its padding; blank is NULL",
            "not_stored": "names, the TWSE 合計 row, the 融券 balances and flows",
        }

    def _dataset_description(self, adapter) -> str:
        return "per-security short sales of borrowed securities"
    ...
    def _write_business(
        ...
        *,
        adapter,
        request: SecuritiesLendingRequest,
        parsed: ParsedSecuritiesLending,
        lineage,
        ...
        written = self._writer.append_securities_lendings(
            connection,
