...
| 39 | PLANNED | 歷史產業分類：交易所產業類別調整公告，PIT 的分類期間（起因：stock-model-selection 的資料需求） |
...
2026-09-26 起，歷史產業分類是 Step 39（owner 決定，起因是 `stock-model-selection` 的資料需求）；與 38-b 的先後另定。
...
狀態：**PLANNED**（owner 2026-09-26 決定加入；與 38-b 的先後另定）。依賴：Step 27（MERGED）、Step 38-a（MERGED）。
...
- **證交所依產業查的每日行情**（`MI_INDEX?type=<類別>`）以**今天**的分類重建歷史：2020-01-02 查得到 2023-07-03 才設立的
  「數位雲端」（3130、6165、8454）與「居家生活」，所以不是當時的分類。
- **legacy**：`stock_info` 只有今天的快照（3130 已是數位雲端）；每日行情原始檔是全市場一張表，不分產業。
- **MOPS 月營收頁**雖然依產業分表，但 MOPS 以今天的狀態重建舊頁面（Step 22-b）。

可以用的：兩個交易所的**產業類別調整公告**。〈上市公司產業類別劃分暨調整要點〉（證交所，最近修正 114-06-09）規定每年定期調整、
以公告發布並報主管機關備查，必要時得個別調整；櫃買中心有對應的〈上櫃公司產業類別劃分暨調整要點〉。

- **證交所**：`rwd/zh/announcement/announcement?keyword=產業類別&response=json` 列出公告（發文日期、發文字號、主旨、id），
  `rwd/zh/announcement/announcement_detail?id=<id>&response=json` 給內文（公告事項含新類別與實施日期）與附件；附件 PDF
  （例如 `1121802250-1.pdf`）逐家列出**原產業別**。2020 年以後查到 109、110、111、112、115 年的年度調整（2、11、3、47、1 家），
  以及 115-08-19 一家公司的個別申請；113、114 年沒有查到。
- **櫃買中心**：`www/zh-tw/bulletin/announcement`（POST `startDate`、`endDate`、`txtKeyword`）列出公告，
  `www/zh-tw/bulletin/annDetail`（POST `content_file`、`docId`）給內文，逐家寫「由『A』調整為『B』」與實施日期。
  109–115 年每年 5 月都有一份（例如 113-05-15 公告、113-06-03 實施、7 家）；興櫃公司的個別調整不在範圍。
- 例：3130 一零四，2023-05-22 公告，2023-07-03 起由「資訊服務業」調整為「數位雲端」。

另外：重建版的 `MI_INDEX?type=` 仍列著已下市的公司（2025-07-01 的金融保險有 2888、2867、2809），可補它們最後已知的產業。

### 設計草案（開工前由 owner 確認）

- **觀測資料：產業類別變更**。一筆公告裡的一次變更一列：`(stock_id, market, source, effective_date)` 為 key，存公告日、
  原類別、新類別與 provenance（公告列表、內文與附件各自的 fetch）。證交所與櫃買中心各一個 source，不合併（§30）。
  少了它，就沒有任何一個「某日的分類是什麼、何時公開」的依據。
- **分類期間在查詢時推出**，不另建期間表（與 `adjusted_prices_pit:v1` 相同的理由）：由今天的分類沿著 PIT 可見的變更往回推，
  每段期間帶 `effective_from`、`effective_to`、`available_at`。API 資料集依需求暫名 `industry-classifications`，
  支援 `start`／`end`、`date=`、`stock_id` 與三個 PIT 參數。
- **產業代碼**：公告只寫名稱；代碼與名稱的對照取自交易所的類別代碼表（例如 `MI_INDEX` 的 `type`、TPEx 的類別代碼），
  名稱改過的（112 年「觀光事業」→「觀光餐旅」）代碼與名稱都要。
- **下市公司最後已知的產業**：由 `MI_INDEX?type=` 補 `stocks.industry`，語意仍是「最後已知」，不是歷史。

### 開工前要定

1. **與 38-b 的先後**。
2. **變更的可取得時點**：公告只有發文日期、沒有時刻。建議保守取發文日隔天 00:00 Asia/Taipei，寫成 release rule（ADR-0020）。
3. **第一次已知變更之前的分類**：建議視為自其生效起日（2020-01-02 或上市日）即公開——當時正在使用的分類本來就是公開的。
4. **接受的缺口**：
   - 公告的完整性無法完全證明（證交所 113、114 年查無公告）。佐證方式：變更鏈一致性（每次的原類別接得上前一次、最後一次的新類別等於今天的分類；
     有變更過的股票漏掉一次會被抓到，從未出現在公告裡的則抓不到）；另可用 Wayback Machine 的 ISIN 清單快照抽查（只作佐證，不作來源，比照 §30.1）。
   - 轉市場的股票在**舊市場**期間的分類，沒有公告證明時留 NULL，不猜。
5. **下市公司的產業**是併進本 step，還是先單獨補。

### 驗收（草案，依需求方的檢查）

- 今天屬於四個新類別的上市股票，在 2023-07-03 之前都是公告附件列的原產業別。
