...
| 39 | 39-a MERGED (#72)；39-b MERGED (#73)；39-c IN REVIEW (#PR) | 歷史產業分類：交易所產業類別調整公告與櫃買類股行情，PIT 的分類期間（起因：stock-model-selection 的資料需求） |
...
2026-09-26 起，歷史產業分類是 Step 39（owner 決定，起因是 `stock-model-selection` 的資料需求），排在 38-b 之前（owner 2026-09-26 決定）。
...
- **現金增資除權日的跳動屬於「其他有記載的市場事件」**：交易所公布的漲跌停價不以除權參考價為中心（audit §4.10），
  市場不反映稀釋時，還原序列會移動認股權的價值。這是 §80 的慣例，owner 2026-09-26 決定保留還原；Fubon 的還原 K 線對現金增資
  完全不調整，差異已分類。
- **未指定 `source` 時只從這個情境在視窗內看得到的價格挑來源**（code review）：看不到的資料不影響答案（§19）。
...
狀態：37-a **MERGED**（#69）；37-b **MERGED**（#70）；37-c **MERGED**（#71）（Step 36 之後、Step 28 之前，owner 2026-09-26 決定）。依賴：Step 27（MERGED）、Step 36（MERGED）、API 的部署（compose 的 `api` 容器、`0.0.0.0:28617`、資料庫只聽本機）。
...
- [x] API：`indices` 接受可重複的 `index_name`（owner 2026-09-26 決定）。在查詢條件裡篩選（`visibility.rows(index_names=...)`），
  只回傳指定的指數；其他資料集照舊 400。一個來源的全部指數整段歷史是 30–50 MB，單一指數約 0.6 MB。
- [x] 個股頁分頁「價格／籌碼」（`#/stock/2330/chips`）。籌碼八個面板共用交易日曆 x 軸、縮放與游標連動：三大法人買賣超、
  法人連續買賣天數、法人累積淨買賣佔發行股數（標明是估計值、不是持股）、外資持股比率、融資餘額、融券餘額、融資融券使用率、借券賣出餘額。
- [x] 股權分散：大戶／中實戶／散戶佔比的週線，游標所在週的 15 個持股分級表，差異數調整與合計照來源發布。
- [x] 市場頁（`#/market`）：加權指數 K 線（`twse_mi_5mins_hist`）、選定指數的收盤走勢（預設櫃買指數）、各指數最新收盤表、
  證交所與櫃買中心的三大法人買賣差額（各自一個面板；合計列只在表中，不畫成長條）。
- [x] 一檔股票轉市場時，所有籌碼資料集照選定的交易所分開，不合併。
- 驗收證據：`docs/step_reports/step-37-b-acceptance-report.md`。

實作中裁決：

- **兩個價量相差很大的量不放在同一個 y 軸**：融資與融券餘額各自一個面板，均線等疊圖照 37-a。
- **法人彙總的合計列不畫成長條**：長條旁邊不能有一條是其他長條的和；合計列照來源發布列在右表。
- **機構顏色跟著機構走**：兩個交易所的同一類法人同色；來源出現新名稱時拒絕，不猜顏色。
- **同一個指數在兩個來源的名稱不同**（`指數:發行量加權股價指數` 與 `指數/臺灣證券交易所:發行量加權股價指數`），各自照來源保留。
- **x 軸涵蓋該交易所所有籌碼資料集的日期**：轉市場前一晚兩個交易所的借券表都列出該股（audit Step 21-b），那天在另一邊是空格。

### 37-c（MERGED，#71）

- [x] 個股頁「基本面」分頁（`#/stock/2330/fundamentals`）。
- [x] 月營收：月營收長條與公司發布的年增率、月增率、累計年增率；最近 12 個月的表（含備註）。沒有公開時間證明的月份留空。
- [x] 財報重點：營業收入、營業毛利、營業利益、本期淨利、歸屬母公司淨利、基本每股盈餘（4000／5900／6900／8200／8610／9750），
  每份財報自己那一期的值；「累計／單季」切換，預設累計；EPS 長條圖與最近 12 季的表。金融業沒有財報時寫明原因。
- [x] 估值：官方（`official-valuations` 的本益比、股價淨值比、殖利率）與計算（`valuation-metrics` 的本益比、近四季 EPS、ROE、
  本益比歷史百分位）分開成各自的面板並標示（CLAUDE.md §53）；計算的面板附 API 回傳的定義。
- [x] 公司行動：交易所結果檔的每一筆事件與條件；來源不發布的欄位與這一筆沒有值分開標示。
- 驗收證據：`docs/step_reports/step-37-c-acceptance-report.md`。

實作中裁決：

- **單季的第四季留空**：年報只申報全年；單季第四季要以全年減前三季累計算出，那是計算，網頁不做（`valuation_metrics:v1` 的 TTM EPS
  在資料中心裡這樣算，那是它的定義）。所以預設顯示累計，每一季都有申報值。
- **只取財報自己那一期的事實**：報告裡的上一年比較數字是這份報告重述的值，不當成上一年自己的值顯示。
- **同一科目同一期間出現兩個不同值就拒絕**，不挑一個顯示。
- **公司行動的條件照來源的位數顯示**（配股率最多十二位小數），不四捨五入到兩位。
- **月營收不按市場分開**：MOPS 把一檔股票整段月營收歸在今天的市場底下（4736 上櫃期間的月份也在 `mops_t21sc03_sii`），它不是分市場的每日資料（code review of #71）。
- **個體報告的母公司淨利讀 8200**：個體報告沒有 8610，與 `valuation_metrics:v1` 同一套規則（code review of #71）。
...
- 新的 API 端點或計算仍不在範圍；唯一的例外是 37-b 替 `indices` 加的 `index_name` 篩選參數（owner 2026-09-26 決定），它只縮小查詢，不是新端點也不是計算。
...
狀態：**39-a MERGED（#72）；39-b MERGED（#73）；39-c IN REVIEW（#PR）**（owner 2026-09-26 決定加入，排在 38-b 之前）。依賴：Step 27（MERGED）、Step 38-a（MERGED）。
起因：`stock-model-selection` 的資料需求（`docs/requests/industry-classifications.md`，2026-09-26）。

### 背景

`stocks.industry` 是今天 ISIN 清單的產業別，就地更新、沒有歷史：

- 證交所 2023-07-03 新增「綠能環保」「數位雲端」「運動休閒」「居家生活」四類，47 家上市公司同日調整；把今天的分類套在那之前的日期，
  用到的是當時還不存在的資訊。
- 2020 年後下市、今天已不在 ISIN 清單的公司，`industry` 是 NULL（audit §4.11；例如 2888 新光金、2867 三商壽）。

下游要以 PIT 的方式用產業：排除金融保險業、同產業相對排名、回測的產業曝險。

### 來源研究（2026-09-26）

不能當歷史用的：

- **證交所依產業查的每日行情**（`MI_INDEX?type=<類別>`）以**今天**的分類重建歷史：2020-01-02 查得到 2023-07-03 才設立的
  「數位雲端」（3130、6165、8454）與「居家生活」，所以不是當時的分類。它仍列著已下市的公司（2025-07-01 的金融保險有 2888、2867、2809），
  而下市公司不會再被調整，所以只拿來取下市公司**最後已知**的產業。
- **legacy**：`stock_info` 只有今天的快照（3130 已是數位雲端）；每日行情原始檔是全市場一張表，不分產業。
- **MOPS 月營收頁**雖然依產業分表，但 MOPS 以今天的狀態重建舊頁面（Step 22-b）。

可以用的：

- **兩個交易所的產業類別調整公告**。〈上市公司產業類別劃分暨調整要點〉（證交所）與〈上櫃公司產業類別劃分暨調整要點〉（櫃買中心）
  規定每年定期調整、以公告發布，必要時得個別調整。
  - **證交所**：`rwd/zh/announcement/announcement?keyword=產業類別&response=json` 列出公告，`announcement_detail?id=<id>&response=json`
    給內文。v1 窗內生效的：109、110、111、112 年的年度調整（2、11、3、47 家）、115-05-08（1 家）、115-08-19 個別申請（1 家）；
    113、114 年查無公告（關鍵字「調整產業」「產業」「類別」結果相同）。內文逐家寫「由『A』改為『B』」與實施日期，
    只有 112 年的原產業別在附件 PDF（`/staticFiles/announcement/announcement/1121802250-1.pdf`）。
  - **櫃買中心**：`www/zh-tw/bulletin/announcement`（POST `startDate`、`endDate`、`txtKeyword`）列出公告，`www/zh-tw/bulletin/annDetail`
    （POST `content_file`、`docId`）給內文。109–115 年每年 5 月一份（4、10、4、56、7、11、13 家上櫃公司）；同樣只有 112 年的原產業別在附件
    （附件 1 是上櫃、附件 2 是興櫃）。興櫃公司的個別調整不在範圍。
- **櫃買中心依類股查的每日行情**（`www/zh-tw/afterTrading/otc?date=&type=<類別>`）是**當日**的分類，不是重建：2022-07-01 的貿易百貨
  有 15 檔、2023-07-03 為 0 檔（併入居家生活）；4419 在 2024-05-31 屬紡織纖維、2024-06-03 屬觀光餐旅；3687 在 2020-05-29 屬文化創意、
  2020-06-01 屬電子商務（代碼 34）——都與公告一致。
- **產業代碼**：ISIN 的產業別代碼表（`isin.twse.com.tw/isin/class_i.jsp?kind=1`，01–38），兩個市場共用，名稱與 `stocks.industry` 相同；
  證交所 `MI_INDEX?type=` 與櫃買 `afterTrading/otc?type=` 用同一套代碼。公告用的名稱不一致（「建材營造」／「建材營造業」、
  「其他」／「其他業」、「生技醫療」、「半導體」、「電子商務業」），由程式常數對應到代碼。
- **類別本身的變動**（112-03-28 兩個交易所的公告，2023-07-03 實施）：新增 35–38；16「觀光事業」改名「觀光餐旅」；櫃買的 34「電子商務」
  併入 36 數位雲端、18「貿易百貨」併入 38 居家生活——被併入的股票在 112 年的調整公告裡逐家列出。
- 例：3130 一零四，2023-05-22 公告，2023-07-03 起由「資訊服務業」調整為「數位雲端」。

### 決定（owner 2026-09-26）

1. **先做 39，再做 38-b。**
2. **變更的可取得時點**：公告只有發文日期、沒有時刻，取發文日**隔天 00:00 Asia/Taipei**，寫成 release rule（ADR-0020）。
   類別改名與合併同樣以其公告的發文日隔天為準。
3. **第一次已知變更之前的分類**：自其生效起日（2020-01-02 或上市日）即公開——當時正在使用的分類本來就是公開的。
4. **接受的缺口與佐證**：
   - 公告的完整性無法完全證明。佐證方式是變更鏈一致性：每次的原類別接得上前一次、最後一次的新類別等於錨點的分類、每段分類在其期間內
     都是存在的類別（例如 2023-07-03 之前不能是 35–38）。有變更過的股票漏掉一次會被抓到，從未出現在公告裡的則抓不到。
   - 上櫃另以類股行情逐檔對帳幾個日期（2020-01-02、各次實施日前後、今天），抓「從未公告的變更」。上市沒有這樣的來源，缺口照實揭露。
   - **已結束的上櫃期間**（轉上市前的上櫃期間、下櫃公司）以最後交易日類股行情的類別為錨點，沿公告往回推（owner 2026-09-26，
     修改原本「沒有公告就留 NULL」）。已結束的上市期間（下市公司）以 `MI_INDEX?type=` 的最後已知類別為錨點。
5. **下市公司的產業**併進本 step，只補**最後已知**，語意寫明不是歷史；它們 2020 年以後的分類期間同樣由公告往回推。
6. **儲存**：兩張表，分類期間在查詢時推出，不另建期間表（與 `adjusted_prices_pit:v1` 相同的理由）。
   - `industry_changes`：公告中的一次變更一列，key `(stock_id, source, effective_date)`，存發文日、發文字號、原類別與新類別（照公告原文），
     以及內文與附件各自的 fetch。來源 `twse_announcement`（上市）、`tpex_announcement`（上櫃），不合併（§30）。
     少了它，就沒有任何「某日的分類是什麼、何時公開」的依據。
   - `industry_observations`：類股行情某日把某檔列在哪一類，key `(stock_id, source, trade_date)`，存產業代碼與 fetch。
     來源 `tpex_otc_quotes`（當日分類）、`twse_mi_index`（今天的分類重建，只當下市公司最後已知）。
     少了它，已結束期間的錨點與對帳沒有可追溯的出處。
   - 程式常數：產業代碼表、公告用名到代碼的對應、改名與類別存續期間、release rule `industry_announcement_next_day@1`。
...
- **39-a — 公告 adapter、`industry_changes`、backfill**：產業代碼常數、release rule、兩個交易所的列表／內文／附件 parser（附件用 pypdf），
  ...
  **MERGED（#72）**：TWSE 6 則公告 65 筆、TPEx 7 則 105 筆，quarantine 0；163 條變更鏈相鄰不一致 0、上市中的 159 條終點都等於今天的
...
- **39-b — 類股行情觀測、`industry_observations`**：兩個交易所依類股查的每日行情 adapter；已結束期間最後交易日的錨點、上櫃的對帳日期；
  ...
  **MERGED（#73）**：一個日期整批查 36 個類別（不查 13，櫃買另查 80）、整批寫或整批 quarantine；錨點日期是最後一個列出該檔的
  ...
  （`docs/step_reports/step-39-b-acceptance-report.md`）。
  **給 39-c**：分類期間照公告的實施日期；上櫃對帳要把 2021-06-01～06-09 這 10 檔歸類為「櫃買行情延後套用」，不是公告缺漏。
- **39-c — 分類期間與 API**：查詢時推出分類期間（`stock_data_center.v2.visibility`），API 資料集 `industry-classifications`
  ...
  依需求方的檢查驗收。
  **IN REVIEW（#PR）**：`stockdc_backfill` 的 2,019 段掛牌期間推出 2,228 段分類期間（5259 沒有：窗內沒有行情），
...
- 今天屬於四個新類別的股票，在 2023-07-03 之前都是公告列的原產業別。
- 2888 在 2025-07-24 下市前是金融保險業；2867 在 2026-09-01 前是金融保險業；2020 年下市的 2448 在 2020 年有分類。
- 每段目前仍有效的分類等於 `stocks` 今天的 `industry`。
- 任一時點 T，`information_as_of = T` 回傳的期間 `available_at` 都不晚於 T。
- 變更鏈一致性檢查零不一致，或每一筆不一致都有分類說明；上櫃對帳日期逐檔一致，或每筆差異都有分類說明。

### 不在範圍

- GICS、次產業、產業鏈；興櫃與創櫃公司的分類。
- 從公司名稱、財報或價格推論產業。
...
- **已存的事件不再抓明細**，除非列表的價格或類型改了（#50 review）；`correction_check` 一律重抓；
  重跑一年只花一個請求
- **明細失敗只擋下那一列**：list 的 fetch 記 `rows_rejected`，該年維持 pending，下次只補那幾頁；
  失敗的事件仍算「有列出」，不會被誤判撤回
- **一年要到隔年 1 月 1 日 00:00 才算完成**：當年的檔案還會列出之後的事件；還沒開始的年份不請求，
  「今天」是台北日期（#50 review）
- 每頁明細各自 commit，不讓一個交易等整年的 HTTP
- **一列來自兩個原始檔時兩個都記**（#50 review）：`fetch_id` 指向列表、`detail_fetch_id` 指向明細頁；
  CHECK 規定恰好 TWT49U／TWTAUU 有值
