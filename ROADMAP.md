...
| 37 | 37-a IN REVIEW (#69)；37-b、37-c PLANNED | 網頁儀表板：以 API 展示資料庫內容（37-a／37-b／37-c，ADR-0029） |
...
狀態：37-a **IN REVIEW**（#69）；37-b、37-c **PLANNED**（Step 36 之後、Step 28 之前，owner 2026-09-26 決定）。依賴：Step 27（MERGED）、Step 36（MERGED）、API 的部署（compose 的 `api` 容器、`0.0.0.0:28617`、資料庫只聽本機）。
