...
狀態：**PLANNED**。依賴：Step 24-a、Step 16。

驗收：

- 376 週全部匯入，每週以其內容日期為 key
- 每個 10 天以上的間隔都解析為農曆新年休市；任何其他缺口都讓匯入失敗
- 2026-07-09 匯入為完整的 4,003 支證券檔案，而不是 1,849 支證券的重建版本
- 不從 `shareholding.bak` 讀取任何東西；它 2023-09-15 之後的 155 個檔案被過濾到約 1,767 支證券
- **2023-10-20 是有記載的來源缺陷**：`20231020.7z` 是在 1.5 MiB 邊界被截斷的下載，比前後週少 562 支證券，且沒有任何官方端點能重新取得。coverage 報告要把它列為已記載的截斷週，而不是我們的缺口，也不得默默補平（owner 決定，2026-09-22）
- 舊系統 `shareholding` 在它有的 340 週上對帳，且對帳要說明舊系統從 2023-09-15 起被過濾的 155 個檔案
- 每週需要一個新的 coverage cadence（`dataset_expected_coverage` 目前只允許 `trading_day` 與 `calendar_month`）
